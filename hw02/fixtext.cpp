...
{
	stream . read (Buffer, BufferSize);//do not use a length indicator for buffer size
	return stream . good ();
}
...
{
	stream . write (Buffer, BufferSize);//do not use a length indicator for buffer size
	return stream . good ();
}
